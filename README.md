...
### macOS
1. [Releases](https://github.com/hongpaul/markflow/releases) 페이지에서 `Markflow-{version}-arm64.dmg` (Apple Silicon) 또는 `Markflow-{version}-x64.dmg` (Intel) 다운로드
2. DMG를 열고 Applications 폴더로 드래그

### Windows
1. `Markflow-{version}-x64.exe` (NSIS 설치 프로그램) 또는 `Markflow-{version}-x64-portable.exe` (포터블) 다운로드
2. 설치 프로그램 실행

### Linux
1. `Markflow-{version}-x64.AppImage` 또는 `markflow_{version}_amd64.deb` 다운로드
2. AppImage: `chmod +x Markflow-*.AppImage && ./Markflow-*.AppImage`
3. deb: `sudo dpkg -i markflow_*.deb`
