...
type BackupType = 'auto' | 'pre-save' | 'manual'
...
export class BackupService {
  ...
  static getInstance(userData: string): BackupService {
  ...
  private async loadMetadata(): Promise<void> {
    try {
      ...
      this.metadata = JSON.parse(raw) as BackupMetadata[]
    } catch {
      this.metadata = []
    }
  }
  ...
  private hashPath(originalPath: string): string {
  ...
  async recoverBackup(id: string): Promise<{ content: string; encoding: Encoding } | null> {
    await this.init()
  ...
  async cleanup(): Promise<void> {
    ...
    const byFile = new Map<string, BackupMetadata[]>()
    for (const m of this.metadata) {
