...
export function registerFsHandlers() {
  ipcMain.handle('fs:readFile', async (_, args) => {
    try {
  ...
  ipcMain.handle('fs:createFile', async (_, args) => {
    try {
  ...
  ipcMain.handle('fs:trash', async (_, args) => {
    try {
  ...
  ipcMain.handle('fs:readDirectory', async (_, args) => {
    try {
  ...
  ipcMain.handle('fs:searchFiles', async (_, args) => {
    const files = []
    const SKIP_DIRS = new Set(['node_modules'])
    const q = args.query.toLowerCase()
    async function walkSearch(dirPath) {
      try {
  ...
  ipcMain.handle('fs:scanMdFolders', async (_, args) => {
    const mdFolders = []
    const SKIP_DIRS = new Set(['node_modules'])
