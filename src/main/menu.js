  ...
  const template = [
      ...
      ? [
          {
            label: 'Machodown',
            submenu: [
              { role: 'about', label: 'Machodown 정보' },
              { type: 'separator' },
              {
                label: '설정…',
                accelerator: 'Cmd+,',
                click: (_item, win) => win?.webContents.send('menu:settings')
              },
              { type: 'separator' },
              { role: 'services', label: '서비스' },
              { type: 'separator' },
              { role: 'hide', label: 'Machodown 가리기' },
              { role: 'hideOthers', label: '다른 항목 가리기' },
              { role: 'unhide', label: '모두 보기' },
              { type: 'separator' },
              { role: 'quit', label: 'Machodown 종료' }
            ]
          }
        ]
      : []),
    ...
    },
    {
      ...
      submenu: [
        ...
        ...(isMac
          ? [
              { role: 'pasteAndMatchStyle', label: '스타일에 맞춰 붙여넣기' },
    ...
    },
    {
      ...
      submenu: [
        ...
        { type: 'separator' },
        {
          label: 'GitHub',
          click: async () => {
            await shell.openExternal('https://github.com/hongmacho/machodown')
          }
        }
      ]
    }
